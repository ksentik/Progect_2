...
def input_int(prompt):
    ...
    Возвращает:
        int или None если введено не число
    """
    try:
...
def input_float(prompt):
    ...
    Возвращает:
        float или None если введено некорректное значение
    """
    try:
...
def handle_add_expense(storage, tree):
    ...
    Запрашивает у пользователя день, сумму и категорию.

    Параметры:
        storage (ExpenseStorage): хранилище расходов
        tree (BudgetTree): дерево категорий
    """
    print("\n--- Добавление расхода ---")
...
def handle_max_day(storage):
    """
    Обрабатывает запрос дня с максимальными расходами.
